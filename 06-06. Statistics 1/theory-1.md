...
function mode(array) {
    ...
    array.forEach( element => {
        ...
        counts[element] = (counts[element] || 0) + 1 // *(1) см внизу
    });

    // поиск наибольшего значения в счетчике
    let maxValue = null;
    let maxCount = 0;
    ...
    return maxValue; // мода

}

let m = mode([17, 12, 205, 305, 326, 12, 51, 305, 20]);
console.log(m);

// console.log(mode([17, 12, 205, 305, 326, 12, 51, 305, 20])); // так тоже можно
...
13.06 продолжение...
# *(1) комм. к ст. 9 Мода.
## объяснение строки `counts[element] = (counts[element] || 0) + 1`:
```js
// представим, что у нас есть список номеров автобусов, которые подъехали к остановке за день
const buses = [17, 12, 205, 305, 326, 12, 51, 305, 20];
// задача: посчитать сколько раз останавливался у остановки каждый автобус 
// для этого берем "коробку" с ячейками-счетчиками (объект counts)
// т.е. counts - пара "номер автобуса" - "количество подкатов"
const counts = {}; // на данный момент в "коробке" пусто

// подъехал автобус 17:
//   смотрим на ячейку counts[17]. коробка "17" - ещё ничего "не знает" про автобус 17. -> undefined
//   т.е. counts[17] на данный момент undefined
//   значит - начинаем отсчет с 0 и прибабляем 1. 
//      (т.е. было ничего: 
//         {}
//       должно стать 1. (потому что автобус подъехал)
//   Итак, начинаем отсчет с 0 и прибабляем 1: 0 + 1 -> 1
//   Записываем: counts[17] = 1
//   Т.е. {"17": 1}; 
// подъехал автобус 12:
//   логика та же самая: undefined -> 0 + 1 -> 1 -> counts[12] = 1
// <...>
// снова подъехал автобус 12
//    у нас уже есть counts[17] === 1 -> 1+1=2 -> counts[17] = 2

// Т.о. || в js это быстрый способ сказать "возьми число counts[element], а если его нет - возьми ноль"
